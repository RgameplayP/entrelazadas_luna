...
function HomePage() {
  ...
  const [productosAleatorios, setProductosAleatorios] = useState([]); // Nuevo estado para productos aleatorios
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/productos`)
      ...
      .then(data => {
        ...
        setProductosAleatorios(shuffled.slice(0, 4));
        
        // Para el carrusel de novedades
  ...
  return (
    <div style={{ backgroundColor: tema === 'dark' ? '#1a1210' : '#F5EDE0', minHeight: '100vh' }}>
      ...
      >
        ...
        <button onClick={anteriorSlide} style={{ position: 'absolute', left: '15px', top: '50%', transform: 'translateY(-50%)', backgroundColor: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)', color: 'white', border: 'none', borderRadius: '50%', width: 'clamp(35px, 8vw, 45px)', height: 'clamp(35px, 8vw, 45px)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s ease', zIndex: 10 }}>
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <polyline points="15 18 9 12 15 6"></polyline>
          </svg>
        </button>
        <button onClick={siguienteSlide} style={{ position: 'absolute', right: '15px', top: '50%', transform: 'translateY(-50%)', backgroundColor: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)', color: 'white', border: 'none', borderRadius: '50%', width: 'clamp(35px, 8vw, 45px)', height: 'clamp(35px, 8vw, 45px)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s ease', zIndex: 10 }}>
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        <div style={{ position: 'absolute', bottom: '15px', left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: '8px', zIndex: 10 }}>
          {carruselSlides.map((_, index) => (
            <div key={index} onClick={() => setSlideActual(index)} style={{ width: 'clamp(6px, 3vw, 10px)', height: 'clamp(6px, 3vw, 10px)', borderRadius: '50%', backgroundColor: slideActual === index ? '#D4A843' : 'rgba(255,255,255,0.5)', cursor: 'pointer', transition: 'all 0.2s ease' }} />
          ))}
        </div>
      </div>

      {/* Sección de colecciones destacadas - CON PRODUCTOS ALEATORIOS */}
      <div style={{ backgroundColor: tema === 'dark' ? '#1a1210' : '#F5EDE0', padding: 'clamp(30px, 8vw, 60px) 20px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 'clamp(15px, 4vw, 30px)' }}>
            {productosAleatorios.map((producto, index) => {
              ...
              return (
                <div 
                  key={producto.id} 
                  style={{ 
                    backgroundColor: tema === 'dark' ? '#2a1f1a' : 'white', 
                    borderRadius: '15px', 
                    overflow: 'hidden', 
                ...
                >
                  <img 
                    ...
                    alt={producto.nombre} 
                    style={{ width: '100%', height: 'clamp(180px, 30vw, 220px)', objectFit: 'cover' }} 
                  ...
                  <div style={{ padding: 'clamp(12px, 4vw, 20px)', textAlign: 'center' }}>
                    <h3 style={{ color: tema === 'dark' ? '#D4A843' : '#6B3E1B', marginBottom: '8px', fontSize: 'clamp(14px, 4vw, 18px)' }}>{producto.nombre}</h3>
                    <p style={{ color: tema === 'dark' ? '#aaa' : '#7A5A3A', fontSize: 'clamp(11px, 3vw, 14px)', marginBottom: '12px', lineHeight: '1.4' }}>
                      {producto.descripcion ? producto.descripcion.substring(0, 80) : ''}...
                    </p>
                    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '15px' }}>
                      <span style={{ fontSize: 'clamp(16px, 4vw, 20px)', fontWeight: 'bold', color: '#A0522D' }}>
                        {precioLimpio} Bs
                      </span>
                      <button 
                        onClick={(e) => {
      ...
      <div style={{ background: 'linear-gradient(135deg, #6B3E1B 0%, #4A2A0E 100%)', color: 'white', textAlign: 'center', padding: 'clamp(30px, 8vw, 60px) 20px' }}>
        <h2 style={{ fontSize: 'clamp(20px, 6vw, 32px)', marginBottom: '10px' }}>✨ Envíos a todo Bolivia ✨</h2>
